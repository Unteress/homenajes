...
new class extends Component
{
    ...
    public function updateProfileInformation()
    {
        ...
        // --- CAMBIO AQUÍ ---
        // Guardamos un mensaje en la sesión para que aparezca después de recargar
        session()->flash('message', 'Perfil actualizado correctamente.');

        // Forzamos la recarga completa de la página (navigate: false es la clave)
        // Esto hará que la foto del header se actualice.
...
<section>
    ...
    <form wire:submit="updateProfileInformation" class="mt-6 space-y-6">
        ...
        <div class="flex items-center gap-4 pt-4 border-t border-gray-100">
            ...
            @if (session('message'))
                <div x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 3000)" class="text-sm text-green-600 font-bold">
                    {{ session('message') }}
                </div>
            @endif
        </div>
